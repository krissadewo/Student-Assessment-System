...
os.Categorybar = zk.$extends(zul.wgt.Div, {
	bind_: function () {
	...
	beforeSize: zk.ie6_ ? function () {
		jq(this.$n("body")).css("width", "").removeClass("os-categorybar-body-scroll");
	}: zk.$void,
	onSize: function(){
		var width = this.$n().offsetWidth;
		//with scorll or not
		if(width < this.childWidth){
			jq(this.$n("left")).addClass("os-categorybar-left-scroll");
			jq(this.$n("right")).addClass("os-categorybar-right-scroll");
			jq(this.$n("body")).addClass("os-categorybar-body-scroll")
								.width(jq.px0(width- zk(this.$n('body')).sumStyles('lr',jq.margins)));
			this._addScollEvent();
		}else{
			jq(this.$n("left")).removeClass("os-categorybar-left-scroll");
			jq(this.$n("right")).removeClass("os-categorybar-right-scroll");
			jq(this.$n("body")).removeClass("os-categorybar-body-scroll")
								.width(jq.px0(width));
...
zk.load('zul.wgt', function () {
os.Category = zk.$extends(zul.wgt.Button, {
	redraw: function (out) {
	...
	doMouseOver_: function (evt) {
		jq(this.$n()).addClass("os-over");
	},
	doMouseOut_: function (evt) {
		if (zk.ie && jq.isAncestor(this.$n(), evt.domEvent.relatedTarget || evt.domEvent.toElement))
			return; //nothing to do
		jq(this.$n()).removeClass("os-over");
	},
	doClick_: function (evt){
		this.$supers('doClick_', arguments);
		jq(this.$n()).addClass("os-seld")
						  .siblings().removeClass("os-seld");
	}
});
});
